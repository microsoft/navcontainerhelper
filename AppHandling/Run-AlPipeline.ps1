...
if ($useCompilerFolder) {
    ...
    $installedAppIds = @(GetAppInfo -AppFiles $existingAppFiles -compilerFolder $compilerFolder -cacheAppinfoPath (Join-Path $packagesFolder 'cache_AppInfo.json') | Select-Object -ExpandProperty 'AppId')
}
else {
