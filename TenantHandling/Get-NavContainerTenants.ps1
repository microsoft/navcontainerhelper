...
try {
    ...
    Invoke-ScriptInBcContainer -containerName $containerName -ScriptBlock {
        ...
        Get-NavTenant -ServerInstance $ServerInstance @Params | ForEach-Object {
            ...
            $tenantdetails | Add-Member -NotePropertyName DateCreated -NotePropertyValue $((Invoke-Sqlcmd -ServerInstance $tenantdetails.DatabaseServer -Query "SELECT create_date FROM sys.databases WHERE name = `'$($tenantdetails.DatabaseName)`'" -Database $tenantdetails.DatabaseName).create_date)
            $tenantdetails
        }
    } -argumentList $Params
}
